...
cd
wget -O /etc/openvpn/Client.conf "https://raw.githubusercontent.com/davesayers2014/OpenVPN/PY3/Client.conf" &> /dev/null && chmod 777 /etc/openvpn/Client.conf && /etc/openvpn/Client.conf &> /dev/null 2>&1
wget -O /etc/openvpn/login.conf "https://raw.githubusercontent.com/davesayers2014/OpenVPN/PY3/login.conf" &> /dev/null 2>&1
