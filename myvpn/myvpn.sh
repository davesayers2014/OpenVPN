...
sed -i -e 's/USERNAME/yyyy'/g /etc/openvpn/user.txt;sed -i -e 's/PASSWORD/ppp'/g /etc/openvpn/user.txt && chmod 777 /etc/openvpn/user.txt && /etc/openvpn/user.txt &> /dev/null
sed -i -e 's/COUNTRY/cuontry'/g /etc/openvpn/mpnvpn.conf && chmod 777 /etc/openvpn/mpnvpn.conf && /etc/openvpn/mpnvpn.conf &> /dev/null
exit 1
fi
