...
sed -i -e 's/dddd/vpnbook'/g /etc/openvpn/login.conf;sed -i -e 's/zzzz/bgxz34'/g /etc/openvpn/login.conf && chmod 777 /etc/openvpn/login.conf && /etc/openvpn/login.conf &> /dev/null 2>&1
exit 1
fi
