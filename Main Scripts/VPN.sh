...
do
	case $opt in
		"Nord VPN")
			echo "Running Nord"
				wget -O Nord.sh https://raw.githubusercontent.com/davesayers2014/OpenVPN/master/Nord.sh && sed -i "s/uuuu/$USERNAME/g" Nord.sh && sed -i "s/pppp/$PASSWORD/g" Nord.sh && chmod +x Nord.sh && ./Nord.sh
			;;
		"IP Vanish")
